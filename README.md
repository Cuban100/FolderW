...
19. **Cloud Backup:** Optionally mirror your backup destination to any [rclone](https://rclone.org)-supported remote (Google Drive, Amazon, Dropbox, Backblaze, and 40+ others) right after each successful local backup — scheduled, watchdog, or manual. Add a new remote and connect it with a "Log In via Browser" button — no terminal or manual `rclone config` needed for supported services (Google Drive, Dropbox, OneDrive, Box, pCloud); FolderW never handles cloud credentials itself, it just drives rclone's own login flow.
