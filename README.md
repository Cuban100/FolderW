...
If setup couldn't configure this automatically (no graphical polkit agent available, `pkexec` failed, etc.), it prints the exact command to run manually — or backups will still work, just skipping (not hanging on) any file they don't have permission to read.
