...
def rsync(destination, link_dest=None, compare_dest=None, result_holder=None):
    ...
    if compare_dest:
        dry_run_command = ["sudo", "-n", "rsync", "-n", "-r", "--out-format=CHANGED:%n", f'--exclude-from={custom_exclude_file}', f'--compare-dest={compare_dest}', src_dir_contents, destination]
    ...
    rsync_command = ["sudo", "-n", "rsync", "-avv", "--sparse", "--info=progress2", "--out-format=CHANGED:%n", f'--exclude-from={custom_exclude_file}', f'--chown={OWNER_USER}:{OWNER_GROUP}', '--chmod=775']
    if link_dest:
